        ...
        if (params) {
            ...
            Object.keys(params).forEach((pn) => {
                function SortStuff(a, b) {
                    let sortKeysNames = params[pn].split(',');
                    var length = sortKeysNames.length;

                    for (let index = 0; index < length; index++) {
                        const sortKeyName = sortKeysNames[index];
                        if (sortKeyName != "desc" && sortKeyName != "asc") {
                            var multiplier = 1;
                            if (index < length + 1 && sortKeysNames[index + 1] == "desc") {
                                multiplier = -1;
                            }
                            var result;
                            result = innerCompare(a[sortKeyName], b[sortKeyName]) * multiplier;
                            if (result != 0) {
                                return result;
                            }
                        }
                ...
                if (pn == "sort") {
                    var test = objectsList.sort((a, b) => SortStuff(a, b))
                    objectsList = test;
                }
                else if (objectsList.length > 0 && objectsList[0][pn]) {
                    var length = objectsList.length;
                    var temp = [];
                    for (let index = 0; index < length; index++) {
                        const element = objectsList[index];
                        if (valueMatch(element[pn], params[pn])){
                            temp.push(element);
                        }
                    }
                    objectsList = temp;
